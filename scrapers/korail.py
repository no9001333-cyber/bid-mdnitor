...
def fetch_korail_bids():
    ...
    results = []
    for item in items:
        title = _get_any(item, "입찰공고명", "공고명", "bidNtceNm", "title", "name")
        if not _matches_keyword(title):
            continue
        deadline = _get_any(item, "개찰일", "개찰일자", "투찰마감일시", "bidClseDt", "endDatetime")
        ...
        results.append({
            ...
            "title": title,
            "org": "한국철도공사",
            "notice_no": _get_any(item, "공고번호", "bidNtceNo", "no"),
            "region": "",
            "base_amount": _get_any(item, "추정가격", "기초금액", "presumedPrice"),
            "notice_date": _get_any(item, "공고개시일", "공고게시일자", "noticeDate"),
            "deadline": deadline,
